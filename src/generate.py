...
def main(args):
    ...
    mode = (
        args.mode
    )  # 运行模式：'cds'（编码序列）、'5utr'（5'非翻译区）或 '3utr'（3'非翻译区）
    ...
    if mode == "cds":
        ...
        if protein_seq is None:
            raise ValueError(
                f"Please provide the protein sequence when using mode {mode}"
            )

        # 检查蛋白质序列是否包含非标准氨基酸
        if has_noncanonical(protein_seq):
            raise ValueError(
                f"The input protein sequence contains non-canonical amino acid characters."
            )
        ...
        model = CDS(
            enc, dec, model_config.prot_pad_idx, model_config.cds_pad_idx, device
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mode",
        type=str,
        help="生成模式：'cds'（编码序列）、'5utr'（5'非翻译区）或 '3utr'（3'非翻译区）",
    )
    parser.add_argument("--ckpt_path", type=str, help="预训练模型检查点文件的路径")
    parser.add_argument(
        "--protein_seq", type=str, help="输入的蛋白质序列（仅 CDS 模式需要）"
    )
    parser.add_argument(
        "--utr_length",
        type=str,
        help="UTR 序列的长度类别：short（短）、medium（中）或 long（长）",
    )
