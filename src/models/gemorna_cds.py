...
class Encoder(nn.Module):
    def __init__(
        ...
        self.device = device
        self.cnn = nn.Conv1d(
            hid_dim, hid_dim, kernel_size=cnn_kernel_size, padding=cnn_padding
        )

    def forward(self, prot, prot_mask):
        batch_size, prot_len = prot.shape
        pos = (
            torch.arange(0, prot_len, device=self.device)
            .unsqueeze(0)
            .expand(batch_size, prot_len)
        )
...
class Decoder(nn.Module):
    ...
    def forward(self, cds, enc_prot, cds_mask, prot_mask):
        ...
        pos = (
            torch.arange(0, cds_len, device=self.device)
            .unsqueeze(0)
            .expand(batch_size, cds_len)
        )
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, prot, prot_mask):
        ...
        normed_attn = self.self_attn_layer_norm(
            prot + self.dropout(self_attention_output)
        )
...
class MultiHeadAttentionLayer(nn.Module):
    def __init__(self, hidden_dim, num_heads, dropout, device):
        ...
        self.dropout = nn.Dropout(dropout)
        self.scale = torch.sqrt(torch.tensor(self.head_dim, dtype=torch.float32)).to(
            device
        )
    ...
    def forward(self, q_input, k_input, v_input, attn_mask=None):
        ...
        def split_heads(tensor):
            return tensor.view(batch_sz, -1, self.n_heads, self.head_dim).permute(
                0, 2, 1, 3
            )
        ...
        merged = (
            weighted_output.permute(0, 2, 1, 3)
            .contiguous()
            .view(batch_sz, -1, self.hid_dim)
        )
        x = self.fc_o(merged)
        return x, attention
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, cds, enc_prot, cds_mask, prot_mask):
        ...
        enc_attn_output, attention = self.encoder_attention(
            x, enc_prot, enc_prot, prot_mask
        )
